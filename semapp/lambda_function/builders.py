...
# TODO: put next two functions into a reader, writer class
s3 = boto3.client('s3', region_name=os.environ.get('REGION_NAME'))

def read_file_from_s3(bucket, file_name_s3):  
    file = BytesIO( s3.get_object( Bucket=bucket, Key=file_name_s3)['Body'].read() )
    return file

def save_df_in_xls_to_s3(df, bucket, file_name_s3):
    xls_buffer = BytesIO()
    df.to_excel(xls_buffer, index = False,  encoding = 'UTF-16', sheet_name='adgroups')
    s3.put_object(Bucket=BUCKET, Key=file_name_s3, Body=xls_buffer.getvalue(), ACL='public-read')
...
def adgroups_builder(file_name_s3, 
    ...
    # read file from S3
    uploaded_file = read_file_from_s3(BUCKET, file_name_s3)
...
#         kw_column = 'processed'
    ...
    s3.put_object(Bucket=BUCKET, Key=output_file_name, Body=xls_buffer.getvalue(), ACL='public-read')
    info['output_file_name'] = output_file_name
    return info


def autobuilder_func(kw_file_name, adgroups_file_name, kw_column, adgroups_column, kw_adgroups_column):
        info = {}
        
        # read files from S3
        kw_file = read_file_from_s3(BUCKET, kw_file_name)
        adgroups_file = read_file_from_s3(BUCKET, adgroups_file_name)
        ...
        if len(kw_df) == 0:
            ...
            return info
        info['kw_count'] = len(kw_df)
        info['adgroups_count'] = len(adgroups_df)
...
#         info['file_saved'] = True
        ...
        # Store file in S3
        results_output = kw_df.drop(columns='embedding_average')
        output_file_name =  kw_file_name[:-5]+'out' +'.xlsx'
        save_df_in_xls_to_s3(results_output, BUCKET, output_file_name)
